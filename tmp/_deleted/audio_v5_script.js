...
function resize() {
    // Always full screen to prevent distortion
...
function loadSettings() {
    const saved = localStorage.getItem('audioVisualizerSettingsV5');
    if (saved) {
        try {
...
function saveSettingsToStorage() {
    if (state.settings.persistSettings) {
        localStorage.setItem('audioVisualizerSettingsV5', JSON.stringify(state.settings));
    }
}
...
function toggleUI() {
    ...
    // No resize needed as canvas is always full screen now
}
...
function startExport() {
    ...
    // Mute output so user doesn't hear it
...
function drawMonitor(fd, drawH) {
    ...
    bands.forEach((b, i) => {
        ...
        // Bar bg
        ctx.fillStyle = '#333';
        ctx.fillRect(boxX + 120, y, 180, 16);
        
        // Bar fill
        const w = (b.val / 255) * 180;
